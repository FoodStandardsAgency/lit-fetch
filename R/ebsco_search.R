...
gen_url_ebsco <- function(searchterm, dateto = Sys.Date() - 1, datefrom = Sys.Date()-365) {
  ...
  datefrom_formatted = format(datefrom,"%Y%m%d")
  dateto_formatted = format(dateto,"%Y%m%d")
  
  query = paste0("(",term,")+AND+(DT+",datefrom_formatted,"-",dateto_formatted,")")
  #"&(TI+AB+SU+yes)"
  
  
  url <- paste0("http://eit.ebscohost.com/Services/SearchService.asmx/Search?",
...
fetch_ebsco <- function(pagenumber, historyinfo) {
  ...
  articlexml <- GET(url) %>% content() %>% xml_nodes("controlInfo")
  
  articleinfo <- map_df(articlexml, function(x) xml2tib(x, "atl,
                                                             ab, 
                                                             ui[type=\"doi\"],
                                                             jtl,
                                                             pubtype,
...
get_ebsco <- function(searchterm,
  ...
  url <- gen_url_ebsco(searchterm, datefrom = datefrom, dateto = dateto)
  
  rcount <- GET(url) %>% 
    content() %>%
    xml2::xml_find_first("//d1:Hits", xml2::xml_ns(.)) %>%
    xml2::xml_text()
  ...
  if(as.numeric(rcount) > 0 ) {
    ...
    results <- map_df(pages, fetch_ebsco, url) %>%
      ...
      mutate(ArticleTitle = {if("atl" %in% names(.)) paste0(atl) else ""}) %>%
      mutate(Abstract = {if("ab" %in% names(.)) paste0(ab) else ""}) %>%
      mutate(ArticleId = {if("ui" %in% names(.)) paste0(ui) else ""}) %>%
      ...
      mutate(PublicationType = {if("pubtype" %in% names(.)) paste0(pubtype) else ""}) %>%
      mutate(Year = {if("dt" %in% names(.)) paste0(dt) else ""}) %>%
      mutate(LastName = {if("LastName" %in% names(.)) paste0(LastName) else ""}) %>%
      mutate(Language = {if("Language" %in% names(.)) paste0(Language) else ""}) %>%
      replace(., . == "NA", "") %>% 
      
      mutate(dt = substr(dt,1,nchar(dt)-3),
             date_format_1 = as.Date(dt,"%m/%d/%Y"),
             #date_format_2 not working
             date_format_2 = ifelse(grepl(".*[a-zA-Z]{3}[0-9]{4}",dt),paste0(substr(dt,1,3),"-",substr(dt,nchar(dt)-3,nchar(dt))),""),
             #date_format_2 not working
             ...
             pdate = case_when(!is.na(date_format_1) ~ date_format_1,
      ...
      mutate(type = "") %>%
      mutate(type = if_else(grepl("Academic Journal", PublicationType), "journal article", type)) %>%
      ...
      ungroup() %>% 
      filter((!is.na(doi) & doi != "" & id == 1) | (title_id == 1 & doi == "") | (title_id == 1 & is.na(doi))) %>% 
      select(-id,-title_id)
