...
pmget <- get_pm(searchtest, datefrom = as.Date("2019-07-01"), dateto = as.Date("2020-06-30"))
...
springurl <- gen_url_springer("botulism", datefrom = as.Date("2019-07-01"), dateto = as.Date("2020-06-30"))

library(httr)
hitcount <- GET(springurl) %>% 
  content(., "text") %>% 
  fromJSON() %>% 
